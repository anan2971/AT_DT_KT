...
def get_data(dataset, max_train_size=None, max_test_size=None, print_log=True, do_preprocess=True, train_start=0,
    ...
    #f = open(os.path.join(prefix1, dataset+'_train.pkl'),"rb")
    f = open("./data/KPI_train.pkl","rb")
    train_data = pickle.load(f).values.reshape((-1, x_dim))[train_start:train_end, :]
    print(type(train_data))
    f.close()
    try:
        #f = open(os.path.join(prefix, dataset + '_test.pkl'), "rb")
        f = open("./data/KPI_test.pkl","rb")
    ...
    try:
        ...
        f = open("./data/KPI_test_label.pkl","rb")
...
def get_data1(dataset, max_train_size=None, max_test_size=None, print_log=True, do_preprocess=True, train_start=0,
             test_start=0):
    """
    ...
    print("test: ", test_start, test_end)
    x_dim = get_data_dim(dataset)
    #f = pd.read_csv("./data/ingress_rate.csv")
    #f = pd.read_csv("./data/app_crash_rate2.csv")
    f = pd.read_csv("./data/all_A4.csv")
    #f = pd.read_csv("./data/KPI.csv")
    train_data=f.values[:, 0]
    num=40000
    #num=len(train_data)
    segmentation=int(0.6*num)
    train_data=train_data[0:segmentation]
    train_data = train_data.reshape(-1, x_dim)
    print(type(train_data))
    #f.close()
    try:
        #f = pd.read_csv("./data/ingress_rate.csv")
        #f = pd.read_csv("./data/app_crash_rate2.csv")
        f = pd.read_csv("./data/all_A4.csv")
        #f = pd.read_csv("./data/KPI1.csv")
        test_data = f.values[:, 0]
        test_data = test_data[segmentation:num]
        test_data = test_data.reshape(-1, x_dim)
        #f.close()
    except (KeyError, FileNotFoundError):
        test_data = None
    try:
        ...
        f = pd.read_csv("./data/all_A4.csv")
        #f = pd.read_csv("./data/KPI1.csv")
        test_label = f.values[:, 1]
        test_label = test_label[segmentation:num]
...
def get_data2(dataset, max_train_size=None, max_test_size=None, print_log=True, do_preprocess=True, train_start=0,
             test_start=0):
    """
    ...
    print("test: ", test_start, test_end)
    x_dim = get_data_dim(dataset)
    #f = pd.read_csv(r"E:\py_projects\pythonProject\Anomaly-Transformer-main\dataset\DIR\ingress_rate.csv")
    train_data=np.load("./data/all_AWS_train.npy")
    ...
    try:
        #f = pd.read_csv(r"E:\py_projects\pythonProject\Anomaly-Transformer-main\dataset\DIR\ingress_rate.csv")
        test_data = np.load("./data/all_AWS_test.npy")
    ...
    try:
        #f = pd.read_csv(r"E:\py_projects\pythonProject\Anomaly-Transformer-main\dataset\DIR\ingress_rate.csv")
        test_label = np.load("./data/all_AWS_test_label.npy")
        test_label = test_label[:12000]
        test_label = test_label.reshape(-1)
        #f.close()
    except (KeyError, FileNotFoundError):
        test_label = None
